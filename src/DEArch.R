...
de = function(dims, range, pop_size, diff_factor, init, select, crossover,
  ...
  for(i in 1:(generations+1)) {
    ...
    if (any_improvement) {
      H_add_unorm = pop_next[[1]][improved,]
      if (length(improved) == 1)
        H_add_unorm = t(H_add_unorm) ### TODO: Remove this ugly hack
      H_unnorm = rbind(H_unnorm, H_add_unorm)
    }

    ###### SHIFTING HISTORY WINDOW
    N = nrow(H_unnorm)
    H_unnorm = H_unnorm[max(1, N - N_history + 1):N, ]
